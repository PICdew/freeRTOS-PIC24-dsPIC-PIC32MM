 ...
 * typically 60-300ns. */
#define LCD_SIGNAL_TIMING         (((configPERIPHERAL_CLOCK_HZ/1000)*1)/1000)

// Define a fast instruction execution time in terms of loop time
// typically > 40us
#define LCD_F_INSTR         (((configPERIPHERAL_CLOCK_HZ/1000)*40)/1000)

// Define a slow instruction execution time in terms of loop time
// typically > 1.64ms
#define LCD_S_INSTR         (((configPERIPHERAL_CLOCK_HZ/1000)*1640)/1000)

// Define the startup time for the LCD in terms of loop time
// typically > 60ms (double than 8 bit mode)
#define LCD_STARTUP         (((configPERIPHERAL_CLOCK_HZ/1000)*60000)/1000)
...
//#if defined(PIM_V0_1)
//#define LCD_D4_Enable()             TRISAbits.TRISA12 = 0;
//#define LCD_D4_Write(value)         LATAbits.LATA12 = value;
//#else
...
unsigned short usRow = 0;
	...
	LCD_Initialize();
	/* Welcome message. */
	LCD_PutString("www.FreeRTOS.org",strlen("www.FreeRTOS.org"));
	...
	for( ;; )
	{
        ...
        LCD_PutString(xMessage.pcMessage,strlen(xMessage.pcMessage));
